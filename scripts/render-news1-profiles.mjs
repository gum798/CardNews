...
const outDir = path.join(root, 'out', 'news1-profiles');
...
// news(소문자) 또는 NEWS(대문자, 레터스페이싱). padding-left로 중앙 보정.
const news = (size, color, caps = false) =>
  `<div style="font-size:${size}px;font-weight:800;color:${color};${caps ? 'letter-spacing:.2em;padding-left:.2em' : 'letter-spacing:-.01em'}">${caps ? 'NEWS' : 'news'}</div>`;

const VARIANTS = {
  '01-stack': wrap(BLUE, col(one(520, '#fff') + news(150, '#fff'), 20)),
  '02-caps': wrap(BLUE, col(one(520, '#fff') + news(130, '#fff', true), 30)),
  '03-inline': wrap(BLUE, `<div style="font-size:240px;font-weight:850;color:#fff;letter-spacing:-.02em">news<span style="margin-left:.15em">1</span></div>`),
  '04-light': wrap('#fff', col(one(520, BLUE) + news(150, '#111318'), 20)),
  '05-badge': wrap(BLUE, col(`<div style="width:460px;height:460px;border:16px solid #fff;border-radius:50%;display:flex;align-items:center;justify-content:center"><span style="font-size:300px;font-weight:900;color:#fff;line-height:1">1</span></div>` + news(120, '#fff', true), 52)),
  '06-bubble': wrap(BLUE, col(`<div style="background:#fff;border-radius:88px;padding:50px 120px;position:relative"><span style="font-size:340px;font-weight:900;color:${BLUE};line-height:1">1</span><div style="position:absolute;bottom:-34px;left:96px;width:0;height:0;border-left:44px solid transparent;border-right:44px solid transparent;border-top:52px solid #fff"></div></div>` + news(130, '#fff'), 60)),
  '07-gradient': wrap(GRAD, col(one(520, '#fff') + news(130, '#fff', true), 30)),
  '08-news-top': wrap(BLUE, col(news(150, '#fff', true) + one(440, '#fff'), 4)),
  '09-navy': wrap(NAVY, col(one(520, '#fff') + news(150, '#4C8DFF'), 20)),
  '10-outline': wrap(BLUE, col(`<div style="font-size:540px;font-weight:900;color:transparent;-webkit-text-stroke:12px #fff;line-height:.9;letter-spacing:-.03em">1</div>` + news(150, '#fff'), 10)),
};
